-- Создание БД 
CREATE DATABASE IF NOT EXISTS add_hw_sem6; -- Создала БД add_seminar6
...
/*
Task1 Создать процедуру, которая решает следующую задачу
...
-- Task2 Создать функцию, вычисляющей коэффициент популярности пользователя (по количеству друзей)
DROP FUNCTION IF EXISTS coeficent_popularity;
DELIMITER //
CREATE FUNCTION coeficent_popularity(user_id INT)
...
END//
DELIMITER ;
SELECT coeficent_popularity(1);

/* Task3 Создайте хранимую функцию hello(), которая будет возвращать приветствие, в зависимости от текущего времени суток.
